...
function recommend(data, employee, limit = 3, model = train(data)) {
  ...
  return data.activities.filter(a => eligible(a, employee, data)).map(activity => {
    ...
    const current = data.grades.find(g => g.id === employee.grade);
    const next = nextGrade(data, employee);
    const goalExplanation = employee.career_goal != null && next ? `Карьерная цель: ${next.role} — ${next.name}. ` : '';
    return { activity, effects, reasons, score, impact, behavior,
      explanation: `${employee.role || current.role || 'Профиль'}: ${current.name} → ${next?.name || '—'}. ${goalExplanation}${behavior.explanation} Критичные навыки имеют двойной вес; оценка учитывает ожидаемое завершение.` };
