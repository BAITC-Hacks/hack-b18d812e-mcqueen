...
const { practicePlan } = require('./practice');
...
function chooseTask(data, employee, source, taskId) {
  ...
  if (taskId !== null) {
    const valid = source === 'catalog' ? employee.available_activities.some(r => r.activity.id === taskId)
      : source === 'practice' && employee.practice.available_tasks.some(task => task.id === taskId);
...
function developmentPlan(data, employee, model = train(data)) {
  ...
  if (!plan.goals.length) {
    ...
    const candidates = data.skills.filter(skill => availableSkills.has(skill.id) && !missing.some(g => g.skill === skill.id && g.gap > 0)).map(skill => {
      ...
      const available = rankForTarget(data, employee, skillTarget, model, label).map(r => ({ ...r, goal_id: id, goal_label: label }));
      return { id, type: 'skill', name: skill.name, label, target: { skill: skill.id, name: skill.name, level }, gaps: skillGaps,
        readiness: Math.round(coverage(skillGaps)), recommendations: available.slice(0, 3), available_activities: available,
...
function dashboard(data) {
  ...
  return data.employees.map(employee => {
    ...
    const available = [...new Map([...development.goals.flatMap(goal => goal.available_activities), ...current].map(r => [r.activity.id, r])).values()].sort((a, b) => b.score - a.score || a.activity.id.localeCompare(b.activity.id));
    const view = { ...employee, next_grade: target, target_source: target ? employee.career_goal != null ? 'career_goal' : 'next_grade' : 'not_set', gaps: missing,
      ...
      recommendations: current.slice(0, 3), available_activities: available, development };
    view.practice = practicePlan(data, view);
    const choice = (data.task_choices || []).find(item => item.employee_id === employee.id);
    const validChoice = choice && (choice.source === 'catalog' ? available.some(r => r.activity.id === choice.task_id) : choice.source === 'practice' && view.practice.available_tasks.some(task => task.id === choice.task_id));
...
module.exports = { validateDataset, gaps, recommend, completeActivity, dashboard, hrSummary, developmentPlan, chooseTask };
